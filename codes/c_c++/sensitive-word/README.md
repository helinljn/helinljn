...
使用 `nanobench` 结合**海量真实本地词库**，对 100 条混合长文本（长度在 15~150 字不等，50% 为正常文本，50% 包含各类脏话变体）进行单线程基准测试，结果如下：
...
**核心指标解读：**
1. **极速响应**：`1 op` 代表处理完 100 条文本。计算下来，**单条长文本的扫描耗时仅为约 32~34 微秒 (µs)**，单线程 QPS 可达近 **30,000 条/秒**。
2. **极佳的算法底座**：完整扫描 (`find_all`) 与提前退出 (`find_first`) 的耗时差距不到 6%，证明底层的 Trie 树寻址机制与“跨度跳跃”算法（避免冗余扫描）效率极高，在安全文本段实现了极速放行。
3. **零分配开销的替换**：得益于底层的 `std::string::reserve` 预分配与精准切片拼接策略，`replace` 替换操作（通常是最耗时的动态内存操作）与 `find_all` 的耗时几乎完全一致，彻底抹平了内存碎片化分配的开销。
4. **极致稳定性**：`err%` 误差率仅为 0.0% ~ 0.2%，表明引擎运行毫无毛刺抖动，不存在偶发性长耗时（Tail Latency），完全满足高可用 SLA 需求。
