...
项目提供统一的文本归一化、黑白名单管理、结果过滤、替换和本地 HTTP 服务能力，能够稳健抵御各种复杂的黑产绕过手段（如繁简变体、特殊符号插入、全半角混用、零宽字符穿透等）。适合在本地服务、工具程序或业务系统中进行敏感内容识别与脱敏处理。
...
**核心指标深度解读：**
1. **百万级别吞吐，微秒级响应**：`1 op` 代表处理完完整的 **100** 条长短混合文本。在 Linux 服务端环境下，`find_first` 的单条长文本拦截响应时间被压缩到了约 **1.5 微秒**！单核吞吐量在 Windows 下达到了 **44 万条/秒**，而在 WSL 下则达到了惊人的 **64 万条/秒**！即便是复杂的全文标记提取或替换，也能轻松跑出单核 **36 万+ QPS**，其性能表现全面碾压任何基于高级语言（如 Java 的垃圾回收和对象逃逸）构建的检测引擎。
2. **提前阻断极其高效**：无论在哪个平台，对比 `find_all`，`find_first` 耗时均缩短了 35% ~ 45%。这表明引擎底层的“短路退出”机制响应极其灵敏。在网关防火墙这种“只要涉敏即时切断”的场景下，可以轻易摸到系统的最高性能上限。
3. **极低额外开销的脱敏替换**：从交叉对比来看，`replace`（文本脱敏重组）的耗时与单纯的 `find_all` 提取结果之间只有不到 6% 的性能损耗。这证明引擎内部极致的内存切片拼接计算以及 `.reserve()` 空间换时间策略，极大地消灭了字符串处理中最致命的“替换引发频繁动态分配与碎片化”这头性能巨兽。
4. **编译器的激进优化与稳如磐石的 SLA**：WSL (Linux) 的速度比 Windows 环境快了近 45%，这不仅归功于 GCC/Clang 编译器在 Linux 环境下对紧凑数组与内联逻辑那极度激进的循环展开优化，也得益于更精简的内存分配子系统。同时 `err%` 的跳动全平台被死死压制在 0.3% ~ 0.9% 内，意味着这套基于连续内存缓存池、纯静态一维数组的内核运行极其平稳，能够为要求极高稳定性的在线服务提供铁一般的基础保障。
5. **硬件亲和性与 L3 缓存敏感**：作为典型的内存密集型 (Memory Bound) 任务，引擎底层的 Trie 树搜索对 CPU 的 **L3 缓存大小**及内存频率极其敏感。得益于 Arena 一维内存池的无碎片化设计，当运行在配备大容量 L3 缓存的现代处理器（如 AMD Zen 3+ 架构或 Intel 大缓存型号）上时，热点节点数据能被整块塞入 CPU 缓存，大幅减少因 Cache Miss 导致的主存访问惩罚，从而爆发出百万级 QPS 的惊人性能。
