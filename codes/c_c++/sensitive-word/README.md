...
### 2.1 Word + Number 统一主扫描流程
项目仍然围绕两类高频检测任务构建：

- **Word**：适合敏感词、违禁词、黑名单词检测
- **Number**：适合长数字串、号码类文本识别

但当前实现上，这两类能力已经统一到 `sensitive_word_engine` 的主扫描流程中：

- 先做一次文本归一化
- 再按起点顺序执行 Word / Number 检测
- 最终复用同一套结果构造、结果过滤、替换逻辑
...
### 2.5 Engine 复制语义清晰
当前 `sensitive_word_engine` 采用显式深拷贝语义：

- 拷贝一个 engine，不再共享同一份内部可变词典状态
- 修改某个副本，不会影响其它副本

这对于运行期动态增删词的场景更安全，也更符合 C++ 用户对值语义对象的直觉。
...
- 逐起点扫描
- 单次文本归一化
- 白名单 / 黑名单统一存储在同一棵 Trie 中
- 节点使用 allow / deny 终止标记区分语义
...
- 运行期动态增删词比较直接
- 行为更容易解释和调试
- 在超大词库、超长文本、高吞吐场景下，仍然不是 AC 自动机那类极限吞吐方案
...
- `word_fail_fast(true)`：优先较快停止当前起点扫描
- `word_fail_fast(false)`：尽量继续探索同一起点上的更长命中

适合在“优先速度”和“优先最长命中”之间做选择。

---

## 5.5 `find_first` 是独立快路径
当前 `find_first` 不再通过 `find_all(..., true)` 间接实现，而是直接走单结果快路径。

这意味着：

- 语义更直接
- 不需要为只取第一个结果而走完整的结果收集流程
- 对“只关心是否命中 / 第一个命中”的场景更友好

---

## 5.6 结果边界判断基于命中上下文
当前结果过滤逻辑（例如英文单词边界判断）不再通过“命中后再回扫原始文本”完成，而是直接基于命中结果携带的上下文信息判断。
...
## 6.3 单次文本归一化结果更轻量
当前 `normalized_text` 已经收缩为主扫描真正需要的数据：

- 只保留 `normalized_chars`
- 不再额外保留整份原始文本副本
- 不再对外暴露冗余的原始 code point 数组
...
`find_first` / `find_all` 返回 `word_result`。

当前 `word_result` 主要字段包括：

- `raw_begin`
- `raw_end`
- `raw_code_point_length`
- `word`
- `normalized_word`
- `type`
...
- `left_raw_code_point`
- `right_raw_code_point`
- `left_normalized_code_point`
- `right_normalized_code_point`

其中：

- `word` 是原始文本中的命中片段
- `normalized_word` 是归一化后的命中内容
- `raw_code_point_length` 常用于替换阶段快速确定替换长度
- 左右边界字段主要用于结果条件判断或更细粒度的后处理逻辑
...
同时，当前代码结构已经收敛到更集中的核心模块：

- `sensitive_word.cpp`
- `text_normalizer.cpp`
- `trie_dictionary.cpp`
- `result_condition.cpp`
- `replace_strategy.cpp`
- `word_dictionary_loader.cpp`

早期独立的 `word_detector` / `num_detector` 模块已经移除，相关能力已内聚回主链路实现。
