...
也可以从文本或文件加载词库（支持 `#` 与 `//` 开头的注释行）：
```cpp
sensitive_word_engine engine = sensitive_word_builder()
    .add_deny_words_from_text("敏感词1\n敏感词2\n# 这是一行注释\n敏感词3")
    .add_deny_words_from_file("res/sensitive_word_deny.txt")
    .add_allow_words_from_file("res/sensitive_word_allow.txt")
    .build();
```
相应地，`add_allow_words_from_text`、`add_allow_words_from_file` 用于加载白名单。
...
// 运行时增删黑名单
engine.add_word("新敏感词");
engine.remove_word("敏感词");

// 运行时增删白名单
engine.add_allow_word("新白名单词");
engine.remove_allow_word("白名单词");
```

#### 查询词条状态
```cpp
sensitive_word::word_entry_status status = engine.query_word_status("敏感词");
// status.exists   → 词条是否存在于字典中
// status.in_deny  → 是否在黑名单
// status.in_allow → 是否在白名单
```

#### 扩展结果条件
除默认的英文单词边界判定 (`english_word_match`) 外，还提供同时检测字母与数字边界的策略：
...
#### 基于预计算结果的替换
先通过 `find_all` 获取命中结果，经业务逻辑调整后，再集中替换：

```cpp
auto results = engine.find_all("天安门和五星红旗");
// ... 可按需过滤、排序或修改 results ...
std::string replaced = engine.replace("天安门和五星红旗", results);       // 默认 *
std::string replaced2 = engine.replace("天安门和五星红旗", results, '#'); // 指定字符
```

#### word_result 详解
`find_first` / `find_all` 返回的 `word_result` 结构体包含完整的上下文信息：

| 字段 | 说明 |
|------|------|
| `word` | 原始文本中的命中片段 |
| `normalized_word` | 命中词的归一化结果 |
| `type` | `match_type::word` 或 `match_type::num` |
| `raw_begin` / `raw_end` | 命中片段在原始文本中的字节区间（右开） |
| `raw_code_point_length` | 命中片段覆盖的原始代码点数量 |
| `left_raw_code_point` / `right_raw_code_point` | 命中片段左右相邻的原始代码点 |
| `left_normalized_code_point` / `right_normalized_code_point` | 命中片段左右相邻代码点的归一化结果 |

左右邻接码点字段主要用于实现自定义 `result_condition` 过滤逻辑（例如判断命中是否处于单词边界）。HTTP `/v1/check` 接口出于精简考虑，不会序列化这 4 个邻接码点字段。
...
如果需要同时对包含数字的命中做边界判定（如 `123` 不应命中 `abc123def`），可使用 `english_word_num_match()`。该策略要求包含 ASCII 字母或数字的命中词左右均为非字母数字字符才算有效命中。
...
Windows 下按以下方式构建（CMake 配置目录为 `.build/windows/x64-Release`，可执行文件及运行资源输出到 `.build/Release`）：
...
`res/` 目录下存放项目敏感词相关资源文件：

| 文件 | 用途 |
|------|------|
| `sensitive_word_deny.txt` | HTTP 服务默认黑名单词库（可按需替换） |
| `sensitive_word_allow.txt` | HTTP 服务默认白名单词库（可按需替换） |
| `dict-2026-04-20.txt` | 由 `scripts/optimize_dict.py` 生成的优化词库样例（76k+ 词条，safe 模式，长度 1~9） |

具体配置可由业务调用方通过 `.add_deny_words_from_file(...)` 灵活加载。

`scripts/optimize_dict.py` 用于从原始词库生成去重、排序、长度过滤后的优化词库。运行该脚本需要安装 `charset-normalizer` 与 `opencc-python-reimplemented`。
