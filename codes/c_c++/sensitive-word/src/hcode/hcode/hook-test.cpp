...
DOCTEST_TEST_SUITE("Hook")
{
    DOCTEST_TEST_CASE("SymbolLoader")
    {
        ...
        // 测试加载动态链接库中的函数
        {
            ...
            DOCTEST_CHECK(lib_loader.load("core.dll"));
    ...
    DOCTEST_TEST_CASE("PatchSharedTestLightHook")
    {
        ...
        // Patch
        {
...
#if defined(CORE_PLATFORM_WINDOWS)
            DOCTEST_CHECK(patch_loader.load("testpatch.dll"));
            DOCTEST_CHECK(testa_loader.load("testa.dll"));
            ...
            // Disable hooks
            DOCTEST_CHECK(disable_hook(&hook_func) != 0);
            DOCTEST_CHECK(disable_hook(&hook_base_func1) != 0);
            DOCTEST_CHECK(disable_hook(&hook_func1) != 0);
            DOCTEST_CHECK(disable_hook(&hook_func2) != 0);
            DOCTEST_CHECK(disable_hook(&hook_func3) != 0);
