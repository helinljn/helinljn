...
TEST_SUITE("FStream")
{
    ...
    TEST_CASE("FStreamWriteAndRead")
    {
        ...
        // 测试 ifstream 读取
        {
            ...
            std::string line;
            CHECK(std::getline(in_file, line));
            CHECK(line == "Hello, fstream!");

            int int_val;
            double double_val;
            in_file >> int_val >> double_val;
            CHECK(int_val == 123);
            CHECK(double_val == doctest::Approx(3.14));
    ...
    TEST_CASE("FStreamReadWrite")
    {
        ...
        // 测试 fstream 读写
        {
            ...
            CHECK(content.find("Hello, fstream!") != std::string::npos);
            CHECK(content.find("Appended content") != std::string::npos);
    ...
    TEST_CASE("FStreamBinary")
    {
        ...
        // 读取二进制数据
        {
            ...
            CHECK(bin_in.is_open());

            int read_data[5];
            bin_in.read(reinterpret_cast<char*>(read_data), sizeof(read_data));
            CHECK(bin_in.gcount() == sizeof(read_data));

            int expected[] = {1, 2, 3, 4, 5};
            for (int i = 0; i < 5; ++i)
            {
                CHECK(read_data[i] == expected[i]);
            }
    ...
    TEST_CASE("FStreamOpenModes")
    {
        ...
        // 验证内容
        {
            ...
            CHECK(content == "Truncated content Appended");
        }
     ...
     * - 验证移动后新对象持有文件句柄
     */
    TEST_CASE("FStreamMoveSemantics")
    {
     ...
     */
    TEST_CASE("FStreamSwapAndCopyfmt")
    {
    ...
    TEST_CASE("FStreamWideStream")
    {
        ...
        // 读取宽字符
        {
            ...
            std::wstring wline;
            CHECK(std::getline(win, wline));
            CHECK(wline == L"Hello, wide fstream!");
    ...
    TEST_CASE("FStreamFormatting")
    {
...

        {
            ...
            // 测试 width()
            out_file.width(10);
            CHECK(out_file.width() == 10);

            // 测试 precision()
            out_file.precision(5);
            CHECK(out_file.precision() == 5);
            ...
            CHECK((current_flags & std::ios::showbase) != 0);
     ...
     */
    TEST_CASE("FStreamRdstateSetstateClear")
    {
...

        {
            ...
            CHECK(state == std::ios::goodbit);

            // 测试 setstate()
            in_file.setstate(std::ios::eofbit);
            state = in_file.rdstate();
            CHECK((state & std::ios::eofbit) != 0);
