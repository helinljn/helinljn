 ...
 ********************************************************************/
TEST_CASE("JsonCpp - Basic Value Creation and Type Checks")
{
    // 默认构造的 Value 为 null
    ...
    Json::Value i(42);
    CHECK(i.isInt());
    CHECK(i.isUInt());      // 42 同时满足 isUInt
    CHECK(i.isIntegral());  // 整数也是 integral
    CHECK(i.isNumeric());   // 整数也是 numeric
    CHECK(i.asInt() == 42);
    CHECK(i.asUInt() == 42u);
    ...
    Json::Value neg(-1);
    CHECK(neg.isInt());
    CHECK(!neg.isUInt());   // 负数不是 UInt
    CHECK(neg.isIntegral());
    CHECK(neg.asInt() == -1);
    ...
    CHECK(i64.isInt64());
    CHECK(i64.isIntegral());
    CHECK(i64.asInt64() == 123456789012LL);

    Json::Value u64(static_cast<Json::UInt64>(123456789012ULL));
    CHECK(u64.isUInt64());
    CHECK(u64.isIntegral());
    CHECK(u64.asUInt64() == 123456789012ULL);

    // 浮点
    Json::Value d(3.14159);
    CHECK(d.isDouble());
    CHECK(d.isNumeric());   // 浮点也是 numeric
    CHECK(!d.isIntegral()); // 浮点不是 integral（除非恰好是整数值，视版本而定）
    CHECK(d.asDouble() == doctest::Approx(3.14159).epsilon(1e-6));
    CHECK(d.asFloat() == doctest::Approx(3.14159f).epsilon(1e-5f));
    ...
    CHECK(nullValue.isConvertibleTo(Json::nullValue));
    CHECK(nullValue.isConvertibleTo(Json::intValue));     // null 可转为 int（结果为 0）
    CHECK(nullValue.isConvertibleTo(Json::booleanValue)); // null 可转为 bool（结果为 false）
    CHECK(!s.isConvertibleTo(Json::intValue));            // 字符串不可转为 int
}

/********************************************************************
 * 测试用例 2：数组操作（append, index, size, clear, 遍历, 越界访问）
 ********************************************************************/
TEST_CASE("JsonCpp - Array operations")
{
    Json::Value arr(Json::arrayValue);
    ...
    CHECK(arr.size() == 4);
    CHECK(arr[0].asInt() == 1);
    CHECK(arr[1].asString() == "two");
    CHECK(arr[2].asBool() == true);
    CHECK(arr[3].asDouble() == doctest::Approx(4.5).epsilon(1e-6));

    // 使用下标赋值可以扩展数组
    arr[4] = "five";
    CHECK(arr.size() == 5);
    CHECK(arr[4].asString() == "five");

    // 越界下标自动填充 null：arr[7] 跳过索引 5、6
    arr[7] = "eight";
    CHECK(arr.size() == 8);
    CHECK(arr[5].isNull());  // 自动填充的中间元素为 null
    CHECK(arr[6].isNull());
    CHECK(arr[7].asString() == "eight");
 ...
 ********************************************************************/
TEST_CASE("JsonCpp - Object operations")
{
    ...
    CHECK(o.isMember("name"));
    CHECK(o["name"].asString() == "Alice");
    CHECK(o["age"].asInt() == 30);
    ...
    addr["zip"] = "100000";
    o["address"] = addr;
    CHECK(o["address"]["city"].asString() == "Beijing");

    // 特殊字符 key（含中文、空格、转义字符）
    o["城市"] = "上海";
    o["key with space"] = "val";
    o["key\"quote"] = "qv";
    CHECK(o["城市"].asString() == "上海");
    CHECK(o["key with space"].asString() == "val");
    CHECK(o["key\"quote"].asString() == "qv");

    // 空字符串 key
    o[""] = "empty_key";
    CHECK(o.isMember(""));
    CHECK(o[""].asString() == "empty_key");
}
 ...
 ********************************************************************/
TEST_CASE("JsonCpp - Parsing from string (CharReaderBuilder)")
{
    ...
    CHECK(root["name"].asString() == "Bob");
    CHECK(root["age"].asInt() == 25);
    CHECK(root["grades"].isArray());
    CHECK(root["grades"][1].asInt() == 92);
    ...
    CHECK(ok3);
    CHECK(dupRoot["x"].asInt() == 2); // 后值覆盖
}

/********************************************************************
 * 测试用例 5：序列化写出（StreamWriterBuilder 为主，标注旧接口 deprecated）
 ********************************************************************/
TEST_CASE("JsonCpp - Writing JSON (StreamWriterBuilder and legacy writers)")
{
    ...
    CHECK(!fastOut.empty());
    CHECK(fastOut.find("\"user\"") != std::string::npos);

    // StyledWriter（已弃用，历史接口）
    Json::StyledWriter styled;
    std::string styledOut = styled.write(root);
    CHECK(!styledOut.empty());
    CHECK(styledOut.find("\n") != std::string::npos);
    ...
    // 默认配置（无缩进，紧凑输出）
    {
        ...
        CHECK(!streamOut.empty());
        CHECK(streamOut.find("\"user\"") != std::string::npos);
    }
    ...
    // 自定义缩进
    {
        ...
        writer->write(root, &oss);
        std::string streamOut = oss.str();
        CHECK(streamOut.find("  ") != std::string::npos); // 应有缩进
        CHECK(streamOut.find("\n") != std::string::npos); // 应有换行
    }
 ...
 ********************************************************************/
TEST_CASE("JsonCpp - Type conversion and exceptions")
{
    Json::Value v;
    ...
    CHECK(v.asInt() == 100);
    CHECK(v.asUInt() == 100u);
    CHECK(v.asDouble() == 100.0);
    CHECK(v.asInt64() == 100);
    CHECK(v.asUInt64() == 100u);
    CHECK(v.asBool() == true);     // 非零整数转为 true
    CHECK(v.asString() == "100");  // 整数转字符串
    ...
    v = "123";
    CHECK(v.isString());
    CHECK(v.asString() == "123");
    CHECK_THROWS_AS(v.asInt(), Json::LogicError);
    CHECK_THROWS_AS(v.asDouble(), Json::LogicError);
    CHECK_THROWS_AS(v.asUInt(), Json::LogicError);
    CHECK_THROWS_AS(v.asBool(), Json::LogicError);

    // 布尔类型：支持隐式转换
    v = true;
    CHECK(v.asBool() == true);
    CHECK(v.asInt() == 1);           // true → 1
    CHECK(v.asDouble() == 1.0);      // true → 1.0
    CHECK(v.asString() == "true");   // true → "true"

    v = false;
    CHECK(v.asBool() == false);
    CHECK(v.asInt() == 0);           // false → 0
    CHECK(v.asString() == "false");  // false → "false"
    ...
    CHECK(v.asBool() == true);
    CHECK(v.asString().find("3.14") != std::string::npos); // 浮点转字符串
    ...
    CHECK_THROWS_AS(v.asInt(), Json::LogicError);
    CHECK_THROWS_AS(v.asString(), Json::LogicError);
 ...
 * 同时验证默认配置下这些特性是禁用的
 ********************************************************************/
TEST_CASE("JsonCpp - Parsing features: comments, trailing commas, single quotes")
{
    ...
    // 显式关闭 allowComments 后应解析失败
    {
        ...
        CHECK(!Json::parseFromStream(b, iss, &r, &e));
    }
 ...
 ********************************************************************/
TEST_CASE("JsonCpp - Error handling and default get")
{
    ...
    CHECK(Json::parseFromStream(builder, iss_json, &root, &errs));
    CHECK(root["age"].isString());
    CHECK_THROWS_AS(root["age"].asInt(), Json::LogicError);

    // get(key, default) 在不存在成员时返回默认值
    const Json::Value& dv = root.get("no", Json::Value("def"));
    CHECK(dv.asString() == "def");

    // get(key, default) 在成员存在时返回实际值（忽略默认值）
    const Json::Value& existing = root.get("age", Json::Value(0));
    CHECK(existing.asString() == "unknown"); // 返回实际值，不是默认值

    // 访问不存在的 key 会自动创建 null 成员（operator[] 的副作用）
    CHECK(root["nonexistent"].isNull());
    CHECK(root.isMember("nonexistent")); // operator[] 已创建该成员
}

/********************************************************************
 * 测试用例 9：复杂嵌套结构的序列化/反序列化 roundtrip
 ********************************************************************/
TEST_CASE("JsonCpp - Complex nested structures and roundtrip")
{
    Json::Value root(Json::objectValue);
    ...
    CHECK(Json::parseFromStream(rb, iss_out, &parsed, &errs));

    // 验证 size
    CHECK(parsed["items"].size() == 3);
    CHECK(parsed["meta"]["version"].asString() == "1.0");

    // 验证具体内容是否一致
    CHECK(parsed["items"][0]["id"].asInt() == 0);
    CHECK(parsed["items"][0]["name"].asString() == "n0");
    CHECK(parsed["items"][1]["id"].asInt() == 1);
    CHECK(parsed["items"][1]["name"].asString() == "n1");
    CHECK(parsed["items"][2]["id"].asInt() == 2);
    CHECK(parsed["items"][2]["name"].asString() == "n2");
}

/********************************************************************
 * 测试用例 10：Json::Path 使用示例（路径解析、默认值、make）
 ********************************************************************/
TEST_CASE("JsonCpp - Json::Path usage")
{
    ...
    CHECK(pName.resolve(r).asString() == "Ann");
    CHECK(pAge.resolve(r).asInt() == 40);
    CHECK(pIdx.resolve(r).asInt() == 20);

    // 默认值示例：resolve(root, defaultValue)
    Json::Value def("missing");
    CHECK(Json::Path(".person.missing").resolve(r, def).asString() == "missing");
    ...
    ref = 42;
    CHECK(root4make["a"]["b"]["c"].asInt() == 42);

    // 路径不存在且无默认值时，resolve 返回 null Value
    CHECK(Json::Path(".nonexistent.path").resolve(r).isNull());
}

/********************************************************************
 * 测试用例 11：大量数据构建与 roundtrip 完整验证
 ********************************************************************/
TEST_CASE("JsonCpp - Large structures and roundtrip sanity")
{
    ...
    CHECK(Json::parseFromStream(rb, iss_s, &parsed, &errs));
    CHECK(parsed.size() == (size_t)N);

    // 验证首尾及中间元素的具体内容
    CHECK(parsed[0]["index"].asInt() == 0);
    CHECK(parsed[0]["text"].asString() == "entry_0");
    CHECK(parsed[N - 1]["index"].asInt() == N - 1);
    CHECK(parsed[N - 1]["text"].asString() == "entry_" + std::to_string(N - 1));
    CHECK(parsed[500]["index"].asInt() == 500);
    CHECK(parsed[500]["text"].asString() == "entry_500");
}

/********************************************************************
 * 测试用例 12：Value 比较（operator==, operator!=）
 ********************************************************************/
TEST_CASE("JsonCpp - Value comparison (operator== and operator!=)")
{
    // 相同值比较
    CHECK(Json::Value(42) == Json::Value(42));
    CHECK(Json::Value(3.14) == Json::Value(3.14));
    CHECK(Json::Value("abc") == Json::Value("abc"));
    CHECK(Json::Value(true) == Json::Value(true));

    // 不同值比较
    CHECK(Json::Value(42) != Json::Value(43));
    CHECK(Json::Value("abc") != Json::Value("def"));
    CHECK(Json::Value(true) != Json::Value(false));

    // 不同类型比较
    CHECK(Json::Value(42) != Json::Value("42"));
    CHECK(Json::Value(1) != Json::Value(true));
 ...
 ********************************************************************/
TEST_CASE("JsonCpp - Value copy, assignment and swap")
{
    // 拷贝构造
    Json::Value original(42);
    Json::Value copied(original);
    CHECK(copied.asInt() == 42);
    CHECK(copied == original);

    // 修改拷贝不影响原值
    copied = 100;
    CHECK(original.asInt() == 42);
    CHECK(copied.asInt() == 100);

    // 赋值运算符
    Json::Value assigned;
    assigned = original;
    CHECK(assigned.asInt() == 42);
 ...
 ********************************************************************/
TEST_CASE("JsonCpp - Unicode and UTF-8 strings")
{
    // 包含中文的 JSON
    ...
    CHECK(Json::parseFromStream(builder, iss, &root, &errs));
    CHECK(root["name"].asString() == "张三");
    CHECK(root["city"].asString() == "北京");
