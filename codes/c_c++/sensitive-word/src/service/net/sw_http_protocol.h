...
using sensitive_word_engine = sensitive_word::sensitive_word_engine;
...
{
    size_t                worker_index;  // worker 下标
    EventLoopPtr          event_loop;    // 所属事件循环
    sensitive_word_engine engine;        // 当前 worker 的本地敏感词引擎
};
...
struct http_result
{
    int         http_status;  // 最终返回的 HTTP 状态码
 ...
 */
std::string make_http_response(int status_code, const std::string& body, bool keep_alive, std::string_view content_type = k_content_type_json);
