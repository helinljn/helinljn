...
// AES-128
//////////////////////////////////////////////////////////////////////////
static inline void AES128_init(AES128_ctx* ctx, const uint8_t* key16)
{
    return AES_setup(ctx->rk, key16, 4, 10);
}

static inline void AES128_encrypt(const AES128_ctx* ctx, size_t blocks, uint8_t* cipher16, const uint8_t* plain16)
{
    while (blocks--)
    {
...
static inline void AES128_decrypt(const AES128_ctx* ctx, size_t blocks, uint8_t* plain16, const uint8_t* cipher16)
{
    while (blocks--)
    {
...
static inline void AES128_destroy(AES128_ctx* ctx)
{
...
// AES-256
//////////////////////////////////////////////////////////////////////////
static inline void AES256_init(AES256_ctx* ctx, const uint8_t* key32)
{
    return AES_setup(ctx->rk, key32, 8, 14);
}

static inline void AES256_encrypt(const AES256_ctx* ctx, size_t blocks, uint8_t* cipher16, const uint8_t* plain16)
{
    while (blocks--)
    {
...
static inline void AES256_decrypt(const AES256_ctx* ctx, size_t blocks, uint8_t* plain16, const uint8_t* cipher16)
{
    while (blocks--)
    {
...
static inline void AES256_destroy(AES256_ctx* ctx)
{
    memset(ctx, 0, sizeof(AES256_ctx));
}

#undef ROT
#undef BIT_RANGE
#undef BIT_RANGE_LEFT
#undef BIT_RANGE_RIGHT

} // namespace aes

void aes128_encrypt(const uint8_t key[16], const uint8_t plain_text[16], uint8_t cipher_text[16])
{
    aes::AES128_ctx ctx;

    aes::AES128_init(&ctx, key);
    aes::AES128_encrypt(&ctx, 1, cipher_text, plain_text);
    aes::AES128_destroy(&ctx);
}

void aes128_decrypt(const uint8_t key[16], const uint8_t cipher_text[16], uint8_t plain_text[16])
{
    aes::AES128_ctx ctx;

    aes::AES128_init(&ctx, key);
    aes::AES128_decrypt(&ctx, 1, plain_text, cipher_text);
    aes::AES128_destroy(&ctx);
}

void aes256_encrypt(const uint8_t key[32], const uint8_t plain_text[16], uint8_t cipher_text[16])
{
    aes::AES256_ctx ctx;

    aes::AES256_init(&ctx, key);
    aes::AES256_encrypt(&ctx, 1, cipher_text, plain_text);
    aes::AES256_destroy(&ctx);
}

void aes256_decrypt(const uint8_t key[32], const uint8_t cipher_text[16], uint8_t plain_text[16])
{
    aes::AES256_ctx ctx;

    aes::AES256_init(&ctx, key);
    aes::AES256_decrypt(&ctx, 1, plain_text, cipher_text);
    aes::AES256_destroy(&ctx);
}
